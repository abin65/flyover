            ...
            <table class="table table-bordered table-hover table-striped glass-table">
                ...
                <tbody>
                    ...
                    $query = "SELECT ja.*, j.job_title, ja.interview_status, ja.apply_status
                                ...
                                WHERE ja.apply_status IN ('approved')  AND ja.interview_status IN ('pending','pass')  and ja.institute_id='$userId' ;
                                ";
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>
                            <form action='schedule_date_update.php' method='POST'>
                                <input type='date' name='scheduled_date' required>
                                <input type='hidden' name='job_apply_id' value='" . $row['job_apply_id'] . "'>
                                <button type='submit' class='btn btn-primary'>Set Date</button>
                            </form>
                          </td>";
                            echo "<td>";
                            echo "<form action='save_google_meet_link.php' method='POST'>";
                            echo "<input type='hidden' name='job_apply_id' value='" . $row['job_apply_id'] . "'>";
                            echo "<input type='url' name='google_meet_link' class='form-control' placeholder='Enter Google Meet link' '>";
                            echo "<button type='submit' class='btn btn-primary mt-2'>Save</button>";
                            echo "</form>";
                            echo "</td>";

                            echo "<td><a href='approve_interview.php?id=" . $row['job_apply_id'] . "' class='btn btn-success'>Approve</a></td>";
                            // Reject button with href
                            echo "<td><a href='reject_interview.php?id=" . $row['job_apply_id'] . "' class='btn btn-danger'>Reject</a></td>";
                            echo "</tr>";
                        }
                    } else {
