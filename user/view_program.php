            ...
            <div class="row">
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        // Add "Apply Now" button for non-expired programs
                        if (!$is_expired) {
                            echo '<button onclick="applyProgram(' . $program_id . ')" class="btn btn-success">Apply Now</button>'; // AJAX apply button
                        }
...
function applyProgram(programId) {
    if (confirm('Are you sure you want to apply for this program?')) {
        $.ajax({
            url: 'program_apply_action.php',
            type: 'POST',
            ...
            error: function(xhr, status, error) {
                alert('Error applying for program: ' + error);
            }
        });
    }
}
